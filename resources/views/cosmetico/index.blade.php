                                ...
                                <tbody>
                                    @foreach ($cosmeticos as $cosmetico)
                                        <tr>
											...
											<td> <img height="50" src="/public/img/{{$cosmetico->imagen}}" alt="Image" > </td>
											<td>{{ $cosmetico->codigo }}</td>

                                            <td>
                                                <form action="{{ route('cosmeticos.destroy',$cosmetico->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('cosmeticos.show',$cosmetico->id) }}"><i class="fa fa-fw fa-eye"></i> Ver</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('cosmeticos.edit',$cosmetico->id) }}"><i class="fa fa-fw fa-edit"></i> Editar</a>
                                                    @csrf
                                                    @method('DELETE')
